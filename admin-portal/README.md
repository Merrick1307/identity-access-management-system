...
The development server proxies `/api` requests to `http://localhost:8000` (the backend).
...
- **GET** `/api/v1/authenticate/sessions/all` — Get all active sessions  
...
- **POST** `/api/v1/users/{id}/password-reset` — Admin-triggered password reset  
